...
function CopyButton({ text, label, variant = "default" }: { text: string; label: string; variant?: "default" | "compact" }) {
    ...
    return (
      ...
      >
        ...
        {copied ? (
          ...
          <svg className="w-3 h-3 flex-shrink-0 opacity-50 group-hover:opacity-100" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    ...
    >
      ...
      {copied ? (
        ...
        <svg className="w-4 h-4 flex-shrink-0 opacity-50 group-hover:opacity-100" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
        </svg>
      )}
    </button>
...
function StepCard({ 
  ...
  return (
    <div className={`relative bg-gradient-to-br ${colors[color]} border rounded-xl p-5 backdrop-blur-sm`}>
      <div className="flex items-start gap-4">
        <div className={`flex-shrink-0 w-8 h-8 ${numberColors[color]} rounded-full flex items-center justify-center font-bold text-sm shadow-lg`}>
          {number}
        </div>
        <div className="flex-1 min-w-0">
          <h4 className={`text-lg font-semibold mb-3 ${colors[color].split(" ").pop()}`}>{title}</h4>
          {children}
        </div>
      </div>
    </div>
...
export default function OGPreviewPage() {
  ...
  const { name, position, about, company, imageUrl, callsign } = profile.personalInfo;
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex flex-wrap items-center gap-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold text-white">OG Banner Generator</h1>
                <p className="text-xs text-slate-400">Create social media preview images</p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4 flex-1">
              ...
              <label className="flex items-center gap-2 cursor-pointer group">
                <div className={`w-10 h-6 rounded-full transition-colors ${showGuides ? 'bg-violet-500' : 'bg-slate-700'} relative`}>
                  <div className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-all ${showGuides ? 'left-5' : 'left-1'}`} />
                </div>
                <span className="text-sm text-slate-400 group-hover:text-slate-300">Guides</span>
                <input
                  type="checkbox"
                  checked={showGuides}
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex justify-center mb-8">
          <div className={`relative rounded-xl overflow-hidden shadow-2xl ${showGuides ? "ring-2 ring-rose-500/50 ring-offset-4 ring-offset-slate-900" : "ring-1 ring-slate-700"}`}>
            <div
              id="og-banner"
              style={{
                width: "1200px",
                ...
                <div className="space-y-4">
                  ...
                  <h2 className="text-3xl font-semibold text-white">
                    ...
                    {company && company.trim() !== "" && (
                      <span className="text-2xl text-slate-300"> @{company}</span>
                    )}
                  </h2>
                  <p className="text-xl text-slate-300 leading-relaxed">{about}</p>
                </div>
            ...
            {showGuides && (
              <div className="absolute inset-0 pointer-events-none">
                ...
                <div className="absolute top-2 left-1/2 -translate-x-1/2 bg-rose-500/80 text-white text-xs px-2 py-0.5 rounded">center</div>
                <div className="absolute top-1/2 left-2 bg-emerald-500/80 text-white text-xs px-2 py-0.5 rounded">safe</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 mb-8 p-4 bg-slate-800/50 rounded-xl border border-slate-700/50">
          ...
          <CopyButton text={`public/images/og-banner.${selectedLang}.webp`} label="Full path" />
          <a 
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-8">
          <StepCard number={1} title="Take Screenshot (PNG)" color="emerald">
            <ol className="space-y-2 text-sm text-slate-300">
              ...
              <li className="flex items-start gap-2">
                ...
                DevTools → Select <CopyButton text="#og-banner" label="selector" variant="compact" />
              </li>
              <li className="flex items-start gap-2">
            ...
            <div className="mt-3 p-2 bg-slate-900/50 rounded-lg">
              ...
              <div className="text-xs text-slate-500 space-y-0.5">
                <div><kbd className="bg-slate-700 px-1 rounded">Ctrl+Shift+S</kbd> Zen/Firefox</div>
                <div><kbd className="bg-slate-700 px-1 rounded">Ctrl+Shift+P</kbd> Chrome</div>
              </div>
            </div>
          </StepCard>
          ...
          <StepCard number={3} title="Add to Project" color="violet">
            <ol className="space-y-2 text-sm text-slate-300">
              ...
              <li className="flex items-start gap-2">
                <span className="text-slate-500">2.</span>
                Copy to <code className="text-violet-400 text-xs">/public/images/</code>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-slate-500">3.</span>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <div className={`text-sm font-medium truncate ${isCompleted ? "text-emerald-400" : "text-slate-300"}`}>
                        {lang.nativeName}
                      </div>
                      <div className="text-xs text-slate-500 font-mono">{lang.code.toUpperCase()}</div>
                    </div>
